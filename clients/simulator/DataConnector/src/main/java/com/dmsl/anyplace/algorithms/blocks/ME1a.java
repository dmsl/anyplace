...
public class ME1a extends AlgorithmBlocks {
	...
	public ME1a(CleanPoi[] vertices, int nBlocks, Random rnd, float probLost) {
		super(3, vertices, nBlocks, probLost);
		this.rnd = rnd;
	}
	...
	public ArrayList<Integer> run(int u) {
		...
		PriorityQueue<ME1aNode> blocksNode = new PriorityQueue<ME1aNode>();
		ArrayList<ME1aNode> localVisited = new ArrayList<>();
		ArrayList<ME1aNode> toReturn = new ArrayList<>();
		double lat1 = Double.parseDouble(vertices[u].getLat());
		double lon1 = Double.parseDouble(vertices[u].getLon());
		double dist = distance(lat1, lon1, lat2, lon2);
		ME1aNode n = new ME1aNode(null, u, 1 - (1 - dist / MAX_DIST));
		blocksNode.add(n);
		localVisited.add(n);

		while (!blocksNode.isEmpty()) {
			ME1aNode current = blocksNode.remove();
			...
			for (ME1aNode ot : getPossibleNodes(current)) {
		...
		// Reverse the path
		Stack<ME1aNode> stack = new Stack<>();
		while (n != null) {
			stack.push(n);
			n = n.parent;
		}
	...
	public PriorityQueue<ME1aNode> getPossibleNodes(ME1aNode n) {
		PriorityQueue<ME1aNode> res = new PriorityQueue<>();
		for (Pair p : vertices[n.u].getNeighboursToList()) {
			...
			double dist = distance(lat1, lon1, lat2, lon2);
			res.add(new ME1aNode(n, p.getTo(), n.h + (1 - (p.getWeight() - (dist / MAX_DIST)))));
	...
	private class ME1aNode implements Comparable<ME1aNode> {
		ME1aNode parent;
		int u;
		double h;
		...
		public ME1aNode(ME1aNode parent, int u, double h) {
			this.parent = parent;
			this.u = u;
			this.h = h;
		...
		public boolean equals(Object obj) {
			ME1aNode o = (ME1aNode) (obj);
			return this.u == o.u;
		}

		@Override
		public int compareTo(ME1aNode o) {
