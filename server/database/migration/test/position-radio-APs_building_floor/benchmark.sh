...
do
	(time exec_curl_mdb "/anyplace/position/radio/APs_building_floor" $jsonObj) &>> exec1.log
done
cat exec1.log | grep real | tr '\n' ' ' | tr '\t' ' ' | sed "s/real //g" | sed "s/0m//g" | tr ' ' ',' | sed "s/^/mongo: /" | sed "s/.$//" > exec.log
echo "" >> exec.log
for ((i=0;i<$EXEC_TIMES;i++))
do
	(time exec_curl_cdb "/anyplace/position/radio/APs_building_floor" $jsonObj) &>> exec2.log
done
