...
def compareJsonArray(cObj, mObj, uKey, keys):
    if len(cObj) != len(mObj):
        ...
        return False
    print("buildings: couchDB = ", len(cObj), " mongoDB =", len(mObj))
...
def compareBuildingId(cObject, mObject):
    keys = ["name", "greeklish", "buildings"]

    buildingSubKeys = ["buid", "address", "coordinates_lat", "coordinates_lon", "description",
                    "geometry", "is_published", "name", "url", "bucode", "username_creator"]
    isSame2 = 1
    for key in keys:
        if key in cObject and key in mObject:
            if key == "buildings":
                if not compareJsonArray(cObject['buildings'], mObject['buildings'], "buid", buildingSubKeys):
                    isSame2 = 3
...
isSame = compareBuildingId(couchObjects, mongoObjects)
if isSame == 1:
