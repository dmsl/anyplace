...
object FileUtils {
  ...
  def getDirFrozenFloor(buid: String, floor_number: String): File = {
    return new File(RADIO_MAPS_FROZEN_DIR + AnyplaceServerAPI.URL_SEPARATOR + buid + AnyplaceServerAPI.URL_SEPARATOR + floor_number)
  }

  def getRadiomapFile(buid: String, floor_number: String): File = {
    return new File(RADIO_MAPS_FROZEN_DIR + AnyplaceServerAPI.URL_SEPARATOR + buid + AnyplaceServerAPI.URL_SEPARATOR +
      floor_number + AnyplaceServerAPI.URL_SEPARATOR + "indoor-radiomap.txt")
  }
  ...
  def getRadioMapFileName(buid: String, floor_number: String): File = {
    return new File(getDirFrozenFloor(buid, floor_number).toString + AnyplaceServerAPI.URL_SEPARATOR + "indoor-radiomap.txt")
  }
